...
class KeyWordGoodsActivity : BaseMvpActivity<GoodsPresenter>(), GoodsView, BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private fun initView() {
        ...
        mAdapter.setOnItemClickListener(object: BaseRecyclerViewAdapter.OnItemClickListener<Goods>{
            override fun onItemClick(item: Goods, position: Int) {
                val intent = Intent(this@KeyWordGoodsActivity, GoodsDetailActivity::class.java)
    ...
    private fun initData() {
        mKeyWord = intent.getStringExtra(GOODS_KEY_WORD)
    }

    private fun loadData(currentPage: Int = 1){
        mMultiStateView.startLoading()
        mKeyWord?.let {
            mPresenter.getGoodsListByKeyWord(it, currentPage)
        }
    }
    ...
    override fun onGetGoodsResult(result: MutableList<Goods>?) {
        ...
        if( mCurrentPage == 1){
            ...
            if (result != null && result.size > 0){
                ...
                Toast.makeText(this@KeyWordGoodsActivity, "没有更多了", Toast.LENGTH_SHORT).show()
            }
        }
    }
